...
const PUBLIC_CAMPAIGN_DETAIL_QUERY_KEY = 'public-campaign-detail';
...
// Get Public Campaign Detail by Identifier
const getPublicCampaignDetail = async (identifier: string): Promise<PublicCampaignDetailResponse> => {
  const { data } = await api.get<PublicCampaignDetailResponse>(`/campaigns/public/business-campaign/${identifier}`);
  return data;
};

export const useGetPublicCampaignDetail = (identifier?: string) => {
  return useQuery({
    queryKey: [PUBLIC_CAMPAIGN_DETAIL_QUERY_KEY, identifier],
    queryFn: () => getPublicCampaignDetail(identifier as string),
    enabled: !!identifier,
  });
};
