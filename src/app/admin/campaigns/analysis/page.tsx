...
import { useGetDetailedCampaignAnalytics, useGetAdminCampaigns } from '@/services/campaigns/hook';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import {
  BarChart, Bar, XAxis, YAxis, Tooltip, ResponsiveContainer, Legend, LineChart, Line, CartesianGrid
} from 'recharts';
import { Loader2, Download, ArrowLeft, Calendar, Search } from 'lucide-react';
...
export default function CampaignAnalysisPage() {
  ...
  const { data: campaignsData, isLoading: isLoadingCampaigns } = useGetAdminCampaigns(1, 100);

  const campaigns = campaignsData?.data || [];

  // Fetch analytics only when a campaign is selected
  const { data: analytics, isLoading: isLoadingAnalytics, error } = useGetDetailedCampaignAnalytics(selectedCampaignId);
  ...
  return (
    <div className="container mx-auto p-6 space-y-8">
      ...
      {!selectedCampaignId ? (
         <div className="flex flex-col items-center justify-center h-[50vh] text-center border-2 border-dashed rounded-lg bg-gray-50">
           <Search className="h-16 w-16 text-gray-300 mb-4" />
           <h3 className="text-lg font-medium text-gray-900">No Campaign Selected</h3>
           <p className="text-gray-500 max-w-sm mt-2">Select a campaign from the dropdown above to view its performance metrics and analytics.</p>
         </div>
      ) : isLoadingAnalytics ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between">
                ...
                <Button size="sm" variant="outline" onClick={() => handleDownload(`${selectedCampaign?.name}_top_rewards`, analytics.topRewards)}>
            ...
            <Card>
               <CardHeader className="flex flex-row items-center justify-between">
                <CardTitle>Top Participants</CardTitle>
                 <Button size="sm" variant="outline" onClick={() => handleDownload(`${selectedCampaign?.name}_top_participants`, analytics.rankedParticipants)}>
                  <Download className="h-4 w-4 mr-2" /> Export
                </Button>
              </CardHeader>
              <CardContent>
