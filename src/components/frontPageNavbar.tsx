  ...
  const pathname = usePathname();
    ...
    const getLinkClass = (href: string) => {
      ...
      return isTransparent
        ? `transition-colors text-white ${isActive ? "font-semibold" : "opacity-90 hover:opacity-100"}`
        : `transition-colors ${isActive ? "text-orange-600" : "text-gray-800 hover:text-orange-600"}`;
    };
    ...
    return (
      ...
      >
        <div className="max-w-7xl mx-auto flex justify-between items-center py-4 px-6">
          <Link href="/">
            <span className={`justify-center items-center text-2xl font-bold ${isTransparent ? "text-white" : "text-orange-500"}`}>
              <span className={`absolute top-4 h-8 w-8 rounded-2xl flex items-center justify-center  ${isTransparent ? "bg-white text-orange-500" : "bg-orange-500 text-white"}`}>
                <span className={` relative left-1.5 pt-0.5 pb-6  h-8 w-8 rounded-2xl text-xl  ${isTransparent ? " text-orange-500" : " text-white"}`}>M
          ...
          <div className="hidden md:flex gap-3">
            {isAuthenticated ? (
              ...
              <>
                <Link href="/login">
                  <span className={`px-5 py-2 border border-orange-500 text-orange-500 rounded-full hover:bg-orange-50 transition ${isTransparent ? "bg-white/90 text-orange-600" : ""}`}>
                    Login
                  </span>
                </Link>
                <Link href="/signup">
