...
export default function FullCampaignPreview({ formData, isMobile = false }: FullCampaignPreviewProps) {
  ...
  return (
    <div className={`bg-gray-50 text-gray-900 pb-24 ${isMobile ? 'min-h-[800px]' : 'min-h-screen'}`}>
      {/* Hero Section */}
      ...
      <div className="max-w-7xl mx-auto px-6 -mt-8 relative z-10 space-y-12">
        ...
        {/* Member Status Banner - Hidden in Preview for now or mock it */}
        {/* {isMember && ( ... )} */}
          ...
          <Card className="bg-white shadow-lg border-none hover:shadow-xl transition-shadow duration-300">
            <CardContent className="p-6 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm font-semibold text-gray-900 capitalize">{formData.audienceType.join(', ') || 'All'}</p>
              </div>
            </CardContent>
          </Card>
          <Card className="bg-white shadow-lg border-none hover:shadow-xl transition-shadow duration-300">
            <CardContent className="p-6 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm font-semibold text-gray-900">
                  {Number(formData.rewardsAvailable) > 0 ? `${formData.rewardsAvailable} Spots` : 'Unlimited'}
                </p>
              </div>
            </CardContent>
          </Card>
        ...
        <div className="flex flex-col gap-16">
          ...
          <div className="max-w-4xl space-y-12">
            ...
            {/* How to Earn Section */}
            {/* Note: CampaignFormData doesn't have explicit howToEarn array usually, 
                it seems the public page expects it from backend response. 
                We might need to construct it or skip it if not in form data.
                Assuming it's not in formData for now as per previous context. 
            */}

            {/* Terms Section */}
            {/* Same for termsAndConditions, if not in formData we skip */}
          </div>
