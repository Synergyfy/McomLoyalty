...
export default function Header() {
  ...
  const campaignIdFromPath = useMemo(() => {
    ...
    if (campaignsIndex === -1) return null;
    const candidate = segments[campaignsIndex + 1];
    const staticSections = new Set(['contact-us', 'earn-points', 'redeem-points', 'my-points']);
    if (candidate && !staticSections.has(candidate)) {
      return candidate;
    }
    return null;
  }, [pathname]);
  ...
  const navLinks = useMemo(() => {
    if (campaignIdFromPath) {
      return [
        { href: `/campaigns/${campaignIdFromPath}/earn-points`, label: 'EARN POINTS' },
        { href: `/campaigns/${campaignIdFromPath}/redeem-points`, label: 'REDEEM POINTS' },
        { href: `/campaigns/${campaignIdFromPath}/contact-us`, label: 'CONTACT US' },
        { href: `/campaigns/${campaignIdFromPath}/my-points`, label: 'MY POINTS' },
      ];
    }
    return [
